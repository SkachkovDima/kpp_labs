...
public class Game extends Application {
  ...
  Image backgroundImg = new Image(
      getClass().getResourceAsStream("background.jpg"));
  ...
  private void update() {
    if (isPressed(KeyCode.UP) && player.getTranslateY() >= MOVE_LENGHT) {
      player.jumpPlayer();
    }
    if (isPressed(KeyCode.LEFT) && player.getTranslateX() >= MOVE_LENGHT) {
      player.setScaleX(-1);
    ...
    if (player.playerVelocity.getY() < TEN) {
